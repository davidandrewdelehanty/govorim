...
// Look a word up, restricted to entries of a given origin.
//
// This split is load-bearing. A hand-written entry is a deliberate correction
// for one word, so it outranks every dictionary. The bulk harvest is the
// opposite: 1886 жаргон senses, and a good share of them are ordinary words
// with a criminal second meaning — язык is "следователь" in there, шеф is
// "главарь шайки", банка is "литр водки". Checked first, that set would hijack
// «язык» in Tolstoy. So the seed runs LAST, after every real dictionary has
// declined the word.
export function lookupGlossary(index, candidates, origin) {
  for (const cand of candidates) {
    const hit = index.get(dictKey(cand));
    if (hit && (!origin || hit.origin === origin)) return { hit: hit, matched: cand };
  }
  return null;
}
